...
let do_add_user name pwd sessionId seqNo = 
  ...
  begin
    ...
    User_table.append uid (User.Add {username=name;pwd=pwd}) "user" sessionId seqNo;
  end
...
let do_block_user me other sessionId seqNo = 
  ...
  let Some other_id = get_user_id_by_name other in
    begin
      ...
      User_table.append my_id (User.RemFollower {follower_id=other_id}) "User" sessionId (seqNo+1); 
     User_table.append other_id (User.RemFollowing {leader_id=my_id}) "User" sessionId (seqNo+1)
    end
...
let do_new_tweet uid str sessionId seqNo = 
  ...
  let tweet_id = Random.int 1000000 (*Uuid.create()*) in
    begin
      ...
      Userline_table.append uid (Userline.NewTweet {tweet_id=tweet_id}) "tweet" sessionId (seqNo+1);
      List.iter 
        (fun fidop -> match fidop with 
           | Some fid -> Timeline_table.append fid (Timeline.NewTweet {tweet_id=tweet_id}) "timeline" sessionId (seqNo+2)
           | None -> ()) fids;
    end
